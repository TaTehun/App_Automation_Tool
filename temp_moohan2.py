...
def test_app_install(device, package_names, app_names, df, install_attempt, launch_attempt):
    ...
    def is_app_installed():
        ...
        #Checking if cancel button is activated for 30 sec
        subprocess.run([
                "adb", "-s", f"{device}", "shell",
    ...
    for i, (package_name, app_name) in enumerate(zip(package_names, app_names)):
        ...
        if is_app_already_installed():
            ...
            os.system(f"adb -s R32Y30003EW uninstall {package_name}")
            time.sleep(3)
                
            if not is_app_already_installed():
                print(package_name, "Uninstalled")
        else:
