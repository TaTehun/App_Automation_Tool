...
def process_csv(csv_filename):
    ...
    package_names = df['App ID/Package name'].str.strip().tolist()
    
    return package_names, df, csv_file
...
def test_app_install(device, package_names, df, install_attempt, launch_attempt, serial, signals, test_stop_flag):
        ...
        try:
            ...
            """            if d.xpath("//*[contains(@text,'Wireless Emergency Alert')]").wait(timeout = 5):
                ...
                if d.xpath("//*[contains(@text,'Allow alerts')]").wait(timeout = 5):
                    d.xpath("//*[contains(@text,'Allow alerts')]").click()
    ...
    def handle_popup():
        ...
        elif d.xpath("//*[contains(@text,'t install')]").exists:
            d.xpath("//*[contains(@text,'Got it')]").click()
        elif d.xpath("//*[contains(@text,'Save username and password')]").exists:
            d.xpath("//*[contains(@text,'Never use Samsung Pass')]").click()
            d.xpath("//*[contains(@text,'OK')]").click()
            #3app.test.mql
        ...
        elif d.xpath("//*[contains(@text,'Cancel')]").exists:
                d(text = "Cancel").click()
        elif d.xpath("//*[contains(@text,'OK')]").exists:
                d(text = "OK").click()
        else:
    ...
    def app_launcher():
        ...
        if d(text = "Close all").wait(timeout=5):
            d(text = "Close all").click()
        time.sleep(1)
        if launch_result[-1] == l_result_list[0]:
        ...
        if l_attempt < launch_attempt / 2:
            ...
            if d(text = "Install").wait(timeout= 10):
                print(f"{package_name} is Uninstalled")
                d(text = "Install").click()
            else: 
                print(f"{package_name} is not deleted")
                    
            for i in range(120): 
                if is_app_already_installed():
                    print(f"{package_name} installed again")
                    break
                elif d(text = "Install").wait(timeout= 10):
                    d(text = "Install").click()
                else:
                    print(f"Failed to re-install {package_name}")
        ...
        try:
            ...
            for i, package_name in enumerate(package_names):
                ...
                for attempt in range(install_attempt):
                    ...
                    if not test_result:
                        print(f"{package_name} installation status: {test_result[-1]}, attempt: {attempt}/{install_attempt}, {remark_list}")
                        if launch_result:
                            launch_result.pop()
                        launch_result.append(l_result_list[1]) # NA
                        test_result.append(t_result_list[1]) # Fail
                    ...
                    if test_result[-1] == t_result_list[0]: #Pass
                        print(f"{package_name} installation status: {test_result[-1]}, attempt: {attempt}/{install_attempt}")
                        if launch_attempt >= 1:
                            for l_attempt in range(launch_attempt):
                                ...
                                if crash_flag.is_set():
                                    ...
                                    print(f"{device},{package_name} launch status: {launch_result[-1]}, attempt: {l_attempt}/{launch_attempt}")
                                    mw_results.clear()
                                    crash_thread.join()
                                    stop_flag.clear()
                                    crash_flag.clear()
                            ...
                            if mw_results:
                                print(package_name, "Multi Window test result:", max(set(mw_results), key=mw_results.count))
                                mw_results.clear()
                            break
                        break
                        ...
                        replace_package_name = package_name.replace(".", "_")
                        ss_file_name = f"{device}_{replace_package_name}.png"
                        ss_file_path = f"/sdcard/{ss_file_name}"
                        subprocess.run(
                        ["adb", "-s", device, "shell", "screencap", "-p", ss_file_path])
                        
                        time.sleep(1)
                        
                        subprocess.run(
                        ["adb", "pull", ss_file_path, ss_file_name])
                ...
                if launch_result:
                    running_result = "Pass" if launch_result[-1] == "Pass" else max(set(launch_result), key=launch_result.count)
                    target_df.at[i, 'Launch Result'] = running_result
                    launch_result.clear()
                if test_result:
            ...
            target_df[saved_columns].to_csv(final_csv, index=False, encoding='utf-8')
            break
...
class AppTesterGUI(QWidget):
    ...
    def load_csv(self):
        try:
            base_dir = get_app_base_dir()
            self.automation_file_path = os.path.join(base_dir, "Install_Launch_Result.csv")
            self.search_file_path = None
            self.resume_file_path = None

            self.package_names, self.df, _ = process_csv(self.automation_file_path)
    ...
    def custom_csv(self): 
        try:
            ...
            self.package_names, self.df, _ = process_csv(self.resume_file_path)
            self.display_data(self.tableWidget)
    ...
    def search_data(self):
        try:
            ...
            self.package_names, self.df, _ = process_csv(csv)
    ...
    def start_testing_all(self):
        try:
            ...
            def run_tests_for_device(device, serial):
                ...
                test_app_install(device, self.package_names, local_df, install_attempt, launch_attempt, serial, self.signals, self.test_stop_flag)
